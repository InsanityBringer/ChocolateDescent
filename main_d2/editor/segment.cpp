...
void med_combine_duplicate_vertices(int8_t *vlp)
{
	int	v,w;
...
int med_attach_segment_rotated(segment *destseg, segment *newseg, int destside, int newside,vms_matrix *attmat)
{
	...
	nsp->segnum = segnum;
	nsp->objects = -1;
	Segment2s[nsp->segnum].matcen_num = -1;
...
//		sp->sides[i].render_flag = 0;
	...
	sp->group = -1;
	Segment2s[sp->segnum].matcen_num = -1;
...
//	sp->scale.z = length;
	...
	Segment2s[sp->segnum].special = 0;
	Segment2s[sp->segnum].value = 0;
	Segment2s[sp->segnum].static_light = 0;
	Segment2s[sp->segnum].matcen_num = -1;
