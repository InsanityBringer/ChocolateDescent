			...
			*segptr = -1;
			...
			while (*segptr != -1)
			{
				...
				for (sidenum=0; sidenum < MAX_SIDES_PER_SEGMENT; sidenum++) 
				{
					...
					if ((wid_result != WID_FLY_FLAG) && (wid_result != 0)) 
					{
						side *sidep = &segp->sides[sidenum];
						int8_t	*vp = Side_to_verts[sidenum];
						int	v;
...
// mprintf((0,"Propagating segments from %i to %i\n",base_seg-Segments,con_seg-Segments));
	...
	Segment2s[con_seg->segnum].static_light = Segment2s[base_seg->segnum].static_light;
...
void copy_uvs_seg_to_seg(segment *destseg,segment *srcseg)
{
	...
	Segment2s[destseg->segnum].static_light = Segment2s[srcseg->segnum].static_light;
}
				...
				if (light_at_point >= 0)
				{
					...
					switch (hit_type) {
						case HIT_NONE:
							...
							Segment2s[rsegp->segnum].static_light += light_at_point;
							if (Segment2s[segp->segnum].static_light < 0)	// if it went negative, saturate
								Segment2s[segp->segnum].static_light = 0;
							break;
						case HIT_WALL:
							break;
						case HIT_OBJECT:
