...
int place_object(segment* segp, vms_vector* object_pos, int object_type)
{
	...
	case OL_WEAPON:
	{
		weapon_info* weapon = &Weapon_info[ObjId[object_type]];

		objnum = obj_create(OBJ_WEAPON, ObjId[object_type], segp - Segments, object_pos,
			&seg_matrix, Polygon_models[weapon->model_num].rad,
			CT_WEAPON, MT_PHYSICS, RT_POLYOBJ);
		...
		//Set polygon-object-specific data 
		obj->rtype.pobj_info.model_num = weapon->model_num;
		obj->rtype.pobj_info.subobj_flags = 0;

		//set Physics info

		vm_vec_zero(&obj->mtype.phys_info.velocity);
		obj->mtype.phys_info.mass = weapon->mass;
		obj->mtype.phys_info.drag = weapon->drag;
		obj->mtype.phys_info.flags |= PF_FREE_SPINNING;
		if (weapon->bounce)
			obj->mtype.phys_info.flags |= PF_BOUNCE;

		obj->mtype.phys_info.rotvel.y = (fix)((P_Rand() - P_Rand()) * 2.5);
		obj->shields = i2f(20);

		//Set control info, based on what gets exported this is just set to itself
		obj->ctype.laser_info.parent_num = objnum;
		obj->ctype.laser_info.parent_signature = obj->signature;
		obj->ctype.laser_info.parent_type = obj->type;
