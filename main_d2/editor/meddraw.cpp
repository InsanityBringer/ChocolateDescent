...
int draw_seg_objects(segment *seg)
{
	int objnum;
...
typedef struct seg_edge {
	...
	uint16_t	type;
	uint8_t		face_count, backface_count;
} seg_edge;
...
void add_edge(int v0,int v1,uint8_t type)
{
	int found;
...
void add_edges(segment *seg)
{
	...
	if (! cc.high) {		//all off screen?
		int	i,sn,fn,vn;
		int	flag;
		uint8_t	edge_flags[N_EDGES_PER_SEGMENT];
...
void draw_edges(int automap_flag)
{
	int i,type;
	seg_edge *e;
...
void draw_mine(segment *mine_ptr,int depth)
{
	...
	edge_list_size = std::min(Num_vertices*4,MAX_EDGES);		//make maybe smaller than max
