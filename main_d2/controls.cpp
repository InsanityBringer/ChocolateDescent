	...
	if (Players[Player_num].flags & PLAYER_FLAGS_AFTERBURNER)
		if (Controls.afterburner_state) //player has key down
		{
			//if (forward_thrust_time >= 0) { 		//..and isn't moving backward
			{
				...
				afterburner_scale = f1_0 + std::min((fix)(f1_0/2),Afterburner_charge) * 2;
		...
		else 
		{
			...
			//charge up to full
			charge_up = std::min(FrameTime/8,f1_0 - Afterburner_charge);	//recharge over 8 seconds
	
			cur_energy = std::max(Players[Player_num].energy-i2f(10),(fix)0);	//don't drop below 10
