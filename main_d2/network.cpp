...
void network_read_endlevel_short_packet(uint8_t* data);
...
void network_send_objects(void)
{
															  ...
															  // over-stressing the receiver.
	{
		...
		if (obj_count_frame) // Send any objects we've buffered
		{
			...
			if (Network_game_type == IPX_GAME)
				ipx_send_internetwork_packet_data((uint8_t*)object_buffer, loc, Network_player_rejoining.player.network.ipx.server, Network_player_rejoining.player.network.ipx.node);
...
//                              mprintf((0, "Sending end marker in packet #%d.\n", frame_num));
				...
				*(short*)(object_buffer + 6) = (obj_count);
				...
				if (Network_game_type == IPX_GAME)
					ipx_send_internetwork_packet_data((uint8_t*)object_buffer, 8, Network_player_rejoining.player.network.ipx.server, Network_player_rejoining.player.network.ipx.node);
...
void network_send_rejoin_sync(int player_num)
{
	...
	if (Network_player_added)
	{
		...
		for (i = 0; i < N_players; i++)
		{
			if ((i != player_num) && (i != Player_num) && (Players[i].connected))
				if (Network_game_type == IPX_GAME)
				{
					ipx_send_packet_data((uint8_t*)&Network_player_rejoining, sizeof(sequence_packet), NetPlayers.players[i].network.ipx.server, NetPlayers.players[i].network.ipx.node, Players[i].net_address);
				}
		}
	}
	...
	if (Network_game_type == IPX_GAME) {
#ifndef MACINTOSH       
		ipx_send_internetwork_packet_data((uint8_t*)&Netgame, sizeof(netgame_info), Network_player_rejoining.player.network.ipx.server, Network_player_rejoining.player.network.ipx.node);
		ipx_send_internetwork_packet_data((uint8_t*)&NetPlayers, sizeof(AllNetPlayers_info), Network_player_rejoining.player.network.ipx.server, Network_player_rejoining.player.network.ipx.node);
#else
...
void network_send_endlevel_short_sub(int from_player_num, int to_player)
{
	...
	if ((to_player != Player_num) && (to_player != from_player_num) && (Players[to_player].connected))
	{
		...
		if (Network_game_type == IPX_GAME)
			ipx_send_packet_data((uint8_t*)&end, sizeof(endlevel_info_short), NetPlayers.players[to_player].network.ipx.server, NetPlayers.players[to_player].network.ipx.node, Players[to_player].net_address);
	}
}
...
network_send_endlevel_sub(int player_num)
{
	...
	for (i = 0; i < N_players; i++)
	{
		if ((i != Player_num) && (i != player_num) && (Players[i].connected)) 
		{
			...
			{
				if (Network_game_type == IPX_GAME)
					ipx_send_packet_data((uint8_t*)&end, sizeof(endlevel_info), NetPlayers.players[i].network.ipx.server, NetPlayers.players[i].network.ipx.node, Players[i].net_address);
			}
		}
	}
}
...
network_send_game_info(sequence_packet* their)
{
	...
	if (!their) 
	{
		if (Network_game_type == IPX_GAME) 
		{
			...
			ipx_send_broadcast_packet_data((uint8_t*)&NetPlayers, sizeof(AllNetPlayers_info));
	...
	else 
	{
		if (Network_game_type == IPX_GAME)
		{    
			ipx_send_internetwork_packet_data((uint8_t*)&Netgame, sizeof(netgame_info), their->player.network.ipx.server, their->player.network.ipx.node);
			ipx_send_internetwork_packet_data((uint8_t*)&NetPlayers, sizeof(AllNetPlayers_info), their->player.network.ipx.server, their->player.network.ipx.node);
		}
	}
...
void network_send_lite_info(sequence_packet* their)
{
	...
	if (!their) 
	{
		if (Network_game_type == IPX_GAME) 
		{
			ipx_send_broadcast_packet_data((uint8_t*)&Netgame, sizeof(lite_info));
	...
	else 
	{
		if (Network_game_type == IPX_GAME) 
		{
			ipx_send_internetwork_packet_data((uint8_t*)&Netgame, sizeof(lite_info), their->player.network.ipx.server, their->player.network.ipx.node);
		}
	}
...
void network_send_netgame_update()
{
	...
	for (i = 0; i < N_players; i++) {
		if ((Players[i].connected) && (i != Player_num))
		{
			if (Network_game_type == IPX_GAME) 
			{
				ipx_send_packet_data((uint8_t*)&Netgame, sizeof(lite_info), NetPlayers.players[i].network.ipx.server, NetPlayers.players[i].network.ipx.node, Players[i].net_address);
			}
		}
	}
...
int network_send_request(void)
{
	...
	if (Network_game_type == IPX_GAME) 
	{
		ipx_send_internetwork_packet_data((uint8_t*)&My_Seq, sizeof(sequence_packet), NetPlayers.players[i].network.ipx.server, NetPlayers.players[i].network.ipx.node);
	}
...
void network_process_gameinfo(uint8_t* data)
{
	...
	memcpy(&Active_games[i], (uint8_t*)newgame, sizeof(netgame_info));
...
void network_process_lite_info(uint8_t* data)
{
	int i, j;
	...
	memcpy(&Active_games[i], (uint8_t*)newgame, sizeof(lite_info));
...
void network_process_packet(uint8_t* data, int length)
{
 ...
 //      if ( length < sizeof(sequence_packet) ) return;
	...
	case PID_GAME_UPDATE:
		...
		if (Network_status == NETSTAT_PLAYING)
		{
			memcpy((uint8_t*)&TempNetInfo, data, sizeof(lite_info));
			if (TempNetInfo.Security == Netgame.Security)
				memcpy(&Netgame, (uint8_t*)&TempNetInfo, sizeof(lite_info));
		}
		if (Game_mode & GM_TEAM)
		{
			int i;
...
void
network_read_endlevel_short_packet(uint8_t* data)
{
	// Special packet for end of level syncing
...
network_send_sync(void)
{
	...
	for (i = 0; i < N_players; i++) 
	{
		...
		if (Network_game_type == IPX_GAME) 
		{
			...
			ipx_send_internetwork_packet_data((uint8_t*)&Netgame, sizeof(netgame_info), NetPlayers.players[i].network.ipx.server, NetPlayers.players[i].network.ipx.node);
			ipx_send_internetwork_packet_data((uint8_t*)&NetPlayers, sizeof(AllNetPlayers_info), NetPlayers.players[i].network.ipx.server, NetPlayers.players[i].network.ipx.node);
		}
...
menu:
	...
	if (Network_status != NETSTAT_PLAYING)
	{
		...
		if (Network_game_type == IPX_GAME) 
		{
			memcpy(me.player.network.ipx.node, ipx_get_my_local_address(), 6);
			memcpy(me.player.network.ipx.server, ipx_get_my_server_address(), 4);
			ipx_send_internetwork_packet_data((uint8_t*)&me, sizeof(sequence_packet), NetPlayers.players[0].network.ipx.server, NetPlayers.players[0].network.ipx.node);
		}
		N_players = 0;
		...
		case PID_GAME_INFO:
			...
			if (NetSecurityFlag == NETSECURITY_WAIT_FOR_GAMEINFO)
			{
				if (TempPlayersInfo->Security == TempNetInfo.Security)
				{
					...
					{
						memcpy(&Active_games[choice], (uint8_t*)&TempNetInfo, sizeof(netgame_info));
...
void network_do_frame(int force, int listen)
{
	...
	if (NakedPacketLen)
	{
		...
		if (Network_game_type == IPX_GAME)
			ipx_send_packet_data((uint8_t*)NakedBuf, NakedPacketLen, NetPlayers.players[NakedPacketDestPlayer].network.ipx.server, NetPlayers.players[NakedPacketDestPlayer].network.ipx.node, Players[NakedPacketDestPlayer].net_address);
#ifdef MACINTOSH
		else
			...
			if (Netgame.ShortPackets)
			{
				...
				for (i = 0; i < N_players; i++)
				{
					if ((Players[i].connected) && (i != Player_num)) 
					{
						...
						if (Network_game_type == IPX_GAME) 
						{
							ipx_send_packet_data((uint8_t*)&ShortSyncPack, sizeof(short_frame_info) - MaxXDataSize + MySyncPack.data_size, NetPlayers.players[i].network.ipx.server, NetPlayers.players[i].network.ipx.node, Players[i].net_address);
						}
					}
				}
			}
			else  // If long packets
			{
				...
				for (i = 0; i < N_players; i++) 
				{
					if ((Players[i].connected) && (i != Player_num)) 
					{
						...
						if (Network_game_type == IPX_GAME)
							ipx_send_packet_data((uint8_t*)&MySyncPack, sizeof(frame_info) - MaxXDataSize + send_data_size, NetPlayers.players[i].network.ipx.server, NetPlayers.players[i].network.ipx.node, Players[i].net_address);
					}
				}
			}
...
void network_ping(uint8_t flag, int pnum)
{
	...
	if (Network_game_type == IPX_GAME)
		ipx_send_packet_data((uint8_t*)mybuf, 2, NetPlayers.players[pnum].network.ipx.server, NetPlayers.players[pnum].network.ipx.node, Players[pnum].net_address);
}
...
void network_send_naked_packet(char* buf, short len, int who)
{
	...
	if (len + NakedPacketLen > MaxXDataSize)
	{
		...
		if (Network_game_type == IPX_GAME)
			ipx_send_packet_data((uint8_t*)NakedBuf, NakedPacketLen, NetPlayers.players[who].network.ipx.server, NetPlayers.players[who].network.ipx.node, Players[who].net_address);
...
sendit:
	...
	ipx_send_internetwork_packet_data((uint8_t*)buf, count, their->player.network.ipx.server, their->player.network.ipx.node);
}
