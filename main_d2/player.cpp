...
void read_player_file(player* plr, FILE* fp)
{
	...
	plr->primary_weapon_flags = file_read_short(fp);
	plr->secondary_weapon_flags = file_read_short(fp);
	for (i = 0; i < MAX_PRIMARY_WEAPONS; i++)
		plr->primary_ammo[i] = file_read_short(fp);
	for (i = 0; i < MAX_SECONDARY_WEAPONS; i++)
		plr->secondary_ammo[i] = file_read_short(fp);
	plr->pad = file_read_short(fp);
	...
	plr->KillGoalCount = file_read_short(fp);
...
void write_player_file(player* plr, FILE* fp)
{
	...
	file_write_short(fp, plr->primary_weapon_flags);
	file_write_short(fp, plr->secondary_weapon_flags);
	for (i = 0; i < MAX_PRIMARY_WEAPONS; i++)
		file_write_short(fp, plr->primary_ammo[i]);
	for (i = 0; i < MAX_SECONDARY_WEAPONS; i++)
		file_write_short(fp, plr->secondary_ammo[i]);
	file_write_short(fp, plr->pad);
	...
	file_write_short(fp, plr->KillGoalCount);
