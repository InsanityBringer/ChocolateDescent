...
extern int pcx_read_bitmap(char* filename, grs_bitmap * bmp, int bitmap_type, uint8_t * palette);

// Writes the bitmap bmp to filename, using palette. Returns error code.

extern int pcx_write_bitmap(char* filename, grs_bitmap* bmp, uint8_t* palette);
