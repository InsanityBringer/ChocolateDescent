...
uint16_t gr_highcolor_clut[256];
...
void gr_palette_create_clut()
{
	...
	for (i = 0; i < 256; i++)
	{
		r = (gr_palette[i * 3] >> 1) & 31;
		g = (gr_palette[i * 3 + 1] >> 1) & 31;
		b = (gr_palette[i * 3 + 2] >> 1) & 31;

		gr_highcolor_clut[i] = (r << 1) | (g << 6) | (b << 11);
	}
}
