...
void ui_listbox_do(UI_GADGET_LISTBOX* listbox, int keypress)
{
	...
	{
		//[ISB] *facedesk* TODO add real fucking timing code. 
		if (B1_PRESSED && listbox->dragging)
		{
			...
			if ((mitem < 0) && (I_GetTicks() > listbox->last_scrolled + 1)) 
			{
				listbox->current_item--;
				listbox->last_scrolled = I_GetTicks();
				listbox->moved = 1;
			}

			if ((mitem >= listbox->num_items_displayed) &&
				(I_GetTicks() > listbox->last_scrolled + 1))
			{
				listbox->current_item++;
				listbox->last_scrolled = I_GetTicks();
				listbox->moved = 1;
