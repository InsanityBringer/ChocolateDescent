...
{
	int			v,s, objnum;
	int8_t			vertex_list[MAX_VERTICES];
...
// ------------------------------------------------------------------------------------------------
void duplicate_group(int8_t *vertex_ids, short *segment_ids, int num_segments)
{
...
int med_copy_group(int delta_flag, segment *base_seg, int base_side, segment *group_seg, int group_side, vms_matrix *orient_matrix)
{
	...
	int 			c;
	int			gs_index;
	int8_t			in_vertex_list[MAX_VERTICES];
	vms_matrix	rotmat;
	int			objnum;
...
int med_move_group(int delta_flag, segment *base_seg, int base_side, segment *group_seg, int group_side, vms_matrix *orient_matrix, int orientation)
{
	...
	int8_t			in_vertex_list[MAX_VERTICES], out_vertex_list[MAX_VERTICES];
	int			local_hvi;
	vms_matrix	rotmat;
...
int rotate_segment_new(vms_angvec *pbh)
{
	...
	create_group_list(Cursegp, &GroupList[ROT_GROUP].segments[0], &GroupList[ROT_GROUP].num_segments, Selected_segs, 0);	// create list of segments in group
...
//		Current_group = 0;
	...
	GroupList[Current_group].num_segments = 0;
	create_group_list(Markedsegp, &GroupList[Current_group].segments[0], &GroupList[Current_group].num_segments, Selected_segs, N_selected_segs);
...
int CreateGroup(void)
{
	...
	//	Create a list of segments to copy.
	GroupList[Current_group].num_segments = 0;
	create_group_list(Markedsegp, &GroupList[Current_group].segments[0], &GroupList[Current_group].num_segments, Selected_segs, 0);
